...
class Storage extends BaseController
{
    ...
    public function show($id)
    {
        if ($this->request->isAJAX()) {
            try {
                $detail = $this->modelDetail->where([$this->model->primaryKey => $id, 'necessary' => 'PENYIMPANAN'])->findAll();
    ...
    public function tableLine($set = null, $detail = [])
    {
        ...
        if (empty($set)) {
            ...
            $dataStorage = $mSparePart->where(['isactive' => 'Y', 'md_category_id' => 100146, 'product_category_id' => $product->getCategoryId()])->findAll();

            $table = [
                $this->field->fieldTable('select', null, 'md_sparepart_id', null, 'required', null, null, $dataStorage, null, 300, 'md_sparepart_id', 'name'),
        ...
        if (!empty($set) && count($detail) > 0) {
            ...
            foreach ($detail as $row) :
                $table[] = [
                    $this->field->fieldTable('select', null, 'md_sparepart_id', null, 'required', null, null, $dataStorage, $row->md_sparepart_id, 300, 'md_sparepart_id', 'name'),
